  ...
  try {
    ...
    try {
      ...
      if (logErr.code === 11000) {
        await UploadLog.updateOne(
          { teacherId, period: 'day', date: today },
          { $inc: { uploadsToday: 1, bytesUsed: bytes } }
        );
      } else {
        console.error('[UPLOAD_LOG_DAY_ERROR]', logErr);
      }
    }

    // ── Update monthly upload log ────────────────────
    // FIX: was `month` (wrong field name), must be `date: month` to match index
    try {
      await UploadLog.findOneAndUpdate(
        { teacherId, period: 'month', date: month },
        { $inc: { uploadsMonth: 1, bytesUsed: bytes } },
        { upsert: true }
      );
    } catch (logErr: any) {
      if (logErr.code === 11000) {
        ...
        console.error('[UPLOAD_LOG_MONTH_ERROR]', logErr);
      }
    }
