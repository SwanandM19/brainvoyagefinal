...
export async function DELETE(req: NextRequest, { params }: Ctx) {
  try {
    ...
    // ── Cascade: wipe all videos this teacher uploaded ────
    if (user.role === 'teacher') {
      const result = await Video.deleteMany({ teacherId: userId });
      console.log(
        `[ADMIN_DELETE_USER] Cascade-deleted ${result.deletedCount} video(s) for teacher ${userId}`
      );
    }
