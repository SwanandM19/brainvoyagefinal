...
// import {
//     Play, Trophy, Search, LogOut, Star, Eye, BookOpen, X,
//     User, TrendingUp, Globe, ChevronLeft, Medal, Crown,
//     Plus, Users, Video, FileText, Image as ImageIcon,
//     Upload, Edit3, Save, Loader2, Trash2, Settings,
//     CheckCircle, Clock, Award, HelpCircle, Menu,
// } from 'lucide-react';
...
//     bio: string; city: string; state: string; yearsOfExperience: number;
// }
// interface Stats {
...
// }
// interface SubInfo {
//     status: string; trialEndsAt: string | null; currentPeriodEnd: string | null;
// }
...
//                     <span className="flex items-center gap-1"><Star size={11} className="text-yellow-400" fill="#facc15" /><span>{video.rating.toFixed(1)}</span></span>
...
//                     <div className="p-5">
//                         <p className="text-[10px] font-extrabold text-[#9CA3AF] uppercase tracking-widest mb-3">Subscription</p>
//                         <div className="flex items-center gap-3">
//                             <div className={`w-3 h-3 rounded-full flex-shrink-0 ${subscription.status === 'active' ? 'bg-green-500' : subscription.status === 'trial' ? 'bg-blue-500' : 'bg-amber-500'}`} />
//                             <span className="text-sm font-extrabold text-[#111827] capitalize">{subscription.status}</span>
...
//                                <div className="relative bg-gradient-to-r from-[#f8fafc] via-[#eef2f7] to-[#e2e8f0] rounded-2xl overflow-hidden border border-gray-200 shadow-sm">

//     <div className="absolute -right-8 -top-8 w-48 h-48 rounded-full bg-orange-200/40 pointer-events-none" />
//     <div className="absolute right-24 -bottom-8 w-32 h-32 rounded-full bg-amber-200/40 pointer-events-none" />
//     <div className="absolute left-1/2 -top-6 w-24 h-24 rounded-full bg-white/60 pointer-events-none" />

//     <div className="relative px-6 pt-5 pb-4">

//         <div className="flex flex-col items-center text-center mb-5">
//             <span className="bg-orange-100 text-orange-600 text-[10px] font-extrabold px-3 py-1 rounded-full border border-orange-200 uppercase tracking-wider mb-2">
//                 ✦ Teacher Community
//             </span>
...
//                 <button
//                     onClick={() => setShowPosting(true)}
//                     className="flex-1 text-left px-5 py-2.5 rounded-full border border-gray-300 text-gray-500 text-sm font-medium hover:bg-gray-100 hover:border-gray-400 hover:text-gray-700 transition-all"
//                 >
//                     Start a post...
//                 </button>
//             </div>
...
//                     className="flex items-center justify-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-bold text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition-all group"
//                 >
...
//                         <Play size={13} className="text-green-600" fill="#4ade80" />
//                     </div>
//                     <span>Video</span>
//                 </button>
...
//                     <Trophy size={9} />
//                     <span>National Rank #{myRank}</span>
//                 </span>
//             </div>
//         )}
            ...
            <div className="space-y-5">
                ...
                <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm overflow-hidden">
                    ...
                    <div className="p-5">
                        <p className="text-[10px] font-extrabold text-[#9CA3AF] uppercase tracking-widest mb-3">Subscription</p>
                        <div className="flex items-center gap-3">
                            <div className={`w-3 h-3 rounded-full flex-shrink-0 ${subInfo.status === 'active' ? 'bg-green-500' : subInfo.status === 'trial' ? 'bg-blue-500' : 'bg-amber-500'}`} />
                            <span className="text-sm font-extrabold text-[#111827] capitalize">{subInfo.status}</span>
                            {subscription.trialEndsAt && subInfo.status === 'trial' && (
                                <span className="text-xs text-[#9CA3AF] ml-auto">
                                    Trial ends {new Date(subscription.trialEndsAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                                </span>
                            )}
                            {subscription.currentPeriodEnd && subInfo.status === 'active' && (
                                <span className="text-xs text-[#9CA3AF] ml-auto">
                                    Renews {new Date(subscription.currentPeriodEnd).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                                </span>
                            )}
                        </div>
                        {/* Cancel Autopay Button — only shown for active subscriptions */}
                                ...
                                <button
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            const data = await res.json();
                                            if (res.ok) {
                                                setSubInfo(prev => ({ ...prev, status: 'cancelled' }));
                                            } else {
